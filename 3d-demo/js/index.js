...
function MainMove() {
  ...
  function scroll(e) {
    ...
    if (target.zoom < 40) target.zoom = 40;
  }
  ...
  mian.addEventListener(
    "touchmove",
    function (e) {
      e.preventDefault();
      if (click) {
        target.rotation.x =
          targetOnDown.rotation.x - (e.touches[0].pageY - cursor.y) * 0.3;
        target.rotation.y =
          targetOnDown.rotation.y + (e.touches[0].pageX - cursor.x) * 0.3;
      }
    },
    { passive: false }
  );
  ...
  (function animate() {
    ...
    perspective.style.transform =
      ...
      perspective.style.webkitTransform =
        "translate3d(" +
